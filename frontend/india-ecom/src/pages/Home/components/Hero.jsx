...
const HeroImage = ({ image , ref }) => {
    ...
    return (
        <div className="grid grid-cols-4 w-screen shrink-0 items-center justify-center" ref={ref}>
            <img
    ...
    return (
        <section className="relative h-[500px]">
            <div className="absolute left-0 top-1/2 z-10">
                <button className="bg-black/55 p-2 rounded-4xl text-white cursor-pointer" onClick={() => scrollPrev()}>
                    <SlArrowLeft size={30} />
                </button>
            </div>
            <div className="overflow-hidden flex flex-nowrap gap-0.5 border-2 h-[500px]" >
                {imageArray && imageArray.map((image , i) => <HeroImage image={image} key={i} ref={(e) => heroCarouselRefs.current[i] = e} />)}
            </div>
            <div className="absolute right-0 top-1/2 z-10">
               <button className="bg-black/55 p-2 rounded-4xl text-white cursor-pointer" onClick={() => scrollNext()}>
